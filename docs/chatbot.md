...
1. **Controllers** (`apps/backend/src/features/chat/controllers/chatController.ts`)
...
2. **Services** (`apps/backend/src/features/chat/services/chatService.ts`)

   - Contains business logic for chat processing
   - Implements product search and query extraction
   - Creates system messages for AI

3. **Repositories** (`apps/backend/src/features/chat/repositories/chatRepository.ts`)
...
4. **Router** (`apps/backend/src/features/chat/router.ts`)
   - Defines and exports routes for the chat feature
   - Configures middleware for chat endpoints

### Frontend Components

1. **Chat Widget** (`apps/shop/src/features/chat/components/ChatWidget.tsx`)

   - Main container component for the chat interface
   - Displays search guidance and loading indicators

2. **Product Chat Hook** (`apps/shop/src/features/chat/hooks/useProductChat.ts`)

   - Custom hook for chat state management
   - Processes streaming data from backend

3. **Product List** (`apps/shop/src/features/chat/components/ChatProductList.tsx`)
