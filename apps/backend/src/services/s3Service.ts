...
export async function assignImageUrlToProducts(
  products: ProductWithImageUrl[]
) {
  // Map over products to get presigned url for each image
...
export async function assignImageUrlToCart(cart: CartWithItems) {
  return {
    ...cart,
    cartItems: await Promise.all(
      cart.cartItems.map(async (cartItem) => ({
        ...
        product: {
          ...cartItem.product,
          images: await Promise.all(
            cartItem.product.images.map(async (image: any) => ({
              ...image,
              imageUrl: await getPresignedUrl({
                bucketName: process.env.AWS_S3_BUCKET_NAME!,
                objectKey: image.imageKey.replace(
