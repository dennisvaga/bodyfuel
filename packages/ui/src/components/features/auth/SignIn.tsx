...
const SignIn = ({ callbackUrl, isAdmin = false }: signInProps) => {
  ...
  const getRedirectUrl = (callbackUrl?: string, isAdmin?: boolean) => {
    // If there's a specific callback URL (like checkout), always use it
    if (callbackUrl && callbackUrl !== "/") {
      return callbackUrl;
    }

    // For admin context, redirect to dashboard
    if (isAdmin) {
      return "/dashboard";
    }

    // Default for regular users from homepage
    return "/";
  };

  useEffect(() => {
    if (isAuthenticated) {
      const redirectUrl = getRedirectUrl(callbackUrl, isAdmin);
      router.push(redirectUrl);
    }
  }, [isAuthenticated, router, callbackUrl, isAdmin]);
            ...
            {!isAdmin &&
              Object.values(providerMap).map((provider) => (
                <Button
                  ...
                  onClick={() => {
                    signIn(provider.id, {
                      callbackUrl: getRedirectUrl(callbackUrl, isAdmin),
                    });
                  }}
                >
